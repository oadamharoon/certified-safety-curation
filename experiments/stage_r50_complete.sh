...
W=${CSC_RUNS}
D=${CSC_WORK}
LOGDIR=$W/logs/r50complete
mkdir -p "$LOGDIR"
one () {
  local t=$1 seed=$2
  local tag="calfilt_lttR50_seed${seed}"
  local key="${t}_${tag}"
...
echo "[$(date +%m/%d-%H:%M:%S)] R50 COMPLETE: $(wc -l < $J) jobs" >> "$LOGDIR/progress.log"
xargs -a "$J" -L1 -P 5 bash -c 'one "$@"' _
echo "[$(date +%m/%d-%H:%M:%S)] R50 COMPLETE DONE ($(ls $LOGDIR/done_* 2>/dev/null | wc -l)/15)" >> "$LOGDIR/progress.log"
