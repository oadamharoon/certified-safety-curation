...
run_cell () {
  ...
  case "$arm" in
    ...
    vfilt_random)   tag="vfilt_random_matchgt_seed$seed"; script=scripts/04p_vfilter_bc.py; extra="SCORE_MODE=random FILTER_FRAC=$(GTF $task) COST_LIMIT=$lim $ens $BCB";;   # 04p requires V_ENSEMBLE_FILE even when it does not load it
    vfilt_return)   tag="vfilt_return_matchgt_seed$seed"; script=scripts/04p_vfilter_bc.py; extra="SCORE_MODE=return FILTER_FRAC=$(GTF $task) COST_LIMIT=$lim $ens $BCB";;
