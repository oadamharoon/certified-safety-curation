#!/bin/bash
# Wait for the in-flight A3 CDT jobs to drain, then rerun the (idempotent) stage.
...
echo "[$(date +%m/%d-%H:%M:%S)] A3 chain: GPU drained, relaunching remaining jobs" >> $W/logs/a3cdt/progress.log
bash $W/scripts/stage_a3_cdt.sh
