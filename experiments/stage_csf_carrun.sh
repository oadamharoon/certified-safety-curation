...
one () {
  ...
  local lim; case $t in *velocity*) lim=20;; *_b) lim=10;; *) lim=25;; esac
  cd $D
...
J=$W/csf_carrun_jobs.txt; : > "$J"
for t in carrun_b; do
  for s in 0 1 2 3 4; do echo "$t $s" >> "$J"; done
done
echo "[$(date +%m/%d-%H:%M:%S)] CSF CARRUN: $(wc -l < $J) jobs" >> "$LOGDIR/progress.log"
xargs -a "$J" -L1 -P 5 bash -c 'one "$@"' _
echo "[$(date +%m/%d-%H:%M:%S)] CSF CARRUN DONE ($(ls $LOGDIR/done_* 2>/dev/null | wc -l)/15)" >> "$LOGDIR/progress.log"
