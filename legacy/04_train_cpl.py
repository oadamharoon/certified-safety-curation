"""Step 04: BC pre-train + CPL fine-tune from VLM preferences."""
...
def main() -> None:
    ...
    # ------------- BC dataset = all active segments -------------
    bc_obs = torch.cat(
    ...
    print(f"Preference pairs: {pref_labels.size(0)} | BC transitions: {bc_obs.size(0)}")
    print(f"obs_dim={obs_dim}, act_dim={act_dim}")

    cpl_dropout = float(cfg.get("cpl_dropout", 0.0))
    policy = GaussianPolicy(obs_dim, act_dim, cfg["hidden_dim"], dropout=cpl_dropout).to(device)
    ...
    out_path = os.path.join(cfg["output_dir"], "cpl_policy.pt")
