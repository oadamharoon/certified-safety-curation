...
shift 2
D=/home/omniverse/workspace/safevlmcpl/vlm-with-cpl/new_data   # the LIVE tree
TMP=$(mktemp -d); trap 'rm -rf "$TMP"; find "$D/outputs/$TASK" -name "*seedchk*" -delete 2>/dev/null' EXIT
/home/omniverse/miniconda3/envs/safevlmcpl/bin/python - "$D/config.yaml" "$TMP/cfg.yaml" <<'PY'
import sys, yaml
...
/home/omniverse/miniconda3/envs/safevlmcpl/bin/python - "$D/outputs/$TASK" <<'PY'
