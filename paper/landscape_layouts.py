...
Rolls the calibrated policy in a fixed layout, bins visited states by (x, y),
...
def main():
    ...
    pck = torch.load(os.path.join(out, "bc_calfilt_ltt_seed0_policy.pt"),
                     map_location="cpu", weights_only=False)
    pol = GaussianPolicy(pck["obs_dim"], pck["act_dim"], pck["hidden_dim"])
    pol.load_state_dict(pck["state_dict"]); pol.eval()
    ...
    for sd in SEEDS:
        ...
        for ep in range(EPISODES):
            ...
            while not done:
                ...
                xs.append(p[0]); ys.append(p[1]); vs.append(v)
                act = a.squeeze(0).numpy()
                act = act + 0.3 * np.random.randn(*act.shape)   # widen coverage
        ...
        panels.append(dict(seed=sd, grid=grid, haz=haz, hr=hr, goal=goal,
                           d=dmin, v=cv, rho=rho, edges=edges))
        print(f"  layout {sd}: {len(cv)} visited cells, cell-level rho = {rho:.3f}", flush=True)
    ...
    cb = fig.colorbar(im, ax=axes[:3], fraction=0.026, pad=0.015)
    ...
    for p, ls in zip(panels, ("-", "--", ":")):
        ...
        ax.plot(bd, bv, ls, lw=1.5, label=f"seed {p['seed']} ($\\rho$ = {p['rho']:.2f})")
    ax.axvline(panels[0]["hr"], color="0.4", ls=":", lw=1)
    ax.text(panels[0]["hr"], ax.get_ylim()[1], " hazard radius", fontsize=6.5,
            color="0.4", va="top", rotation=90)
    ...
    json.dump({str(p["seed"]): {"rho_cell": p["rho"], "n_cells": int(len(p["v"]))}
               for p in panels},
